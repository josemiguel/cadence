...
def _profile_for(documents: list[str], name: str = "corpus") -> SyntacticProfile:
    ...
    key = hashlib.sha256("\x00".join(docs).encode("utf-8")).hexdigest()
    prof = _profiles.get(key)
    if prof is None:
        prof = build_profile(docs, name=name)
        _profiles[key] = prof
    return prof


@server.tool()
def measure_voice(documents: list[str], name: str = "corpus") -> dict:
    """Measure the shape of someone's writing.

    Pass several pieces the same person wrote. Returns the profile as numbers,
    the prose specification a model can write against, and any warnings about
    the corpus being too thin to trust. Below about twelve sentences the spread
    is noise, and the warnings will say so.
    """
    prof = _profile_for(documents, name)
    return {
...
def restyle(draft: str, documents: list[str], model: str = DEFAULT_MODEL,
    ...
    dropped. Needs ANTHROPIC_API_KEY in the environment the server started in.
    """
    prof = _profile_for(documents)
    try:
...
@server.tool()
def check_fidelity(source: str, output: str) -> dict:
    ...
    """
    return fidelity_of(source, output).as_dict()


@server.tool()
def score(text: str, documents: list[str], source: str | None = None) -> dict:
...
@server.tool()
def repair_structure(text: str) -> dict:
